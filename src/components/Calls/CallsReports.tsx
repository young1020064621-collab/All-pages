...
import DashboardMeetingFilter from './DashboardMeetingFilter';
import DashboardTaskFilterPopup from './DashboardTaskFilterPopup';
...
const CallsReport: React.FC = () => {
  ...
  // Filter popup states
  const [showCallsFilter, setShowCallsFilter] = useState(false);
  const [showCoachingFilter, setShowCoachingFilter] = useState(false);
  
  // Filter handlers
  const handleCallsFilterClick = () => {
    setShowCallsFilter(true);
  };
  ...
  const handleCloseCallsFilter = () => {
    setShowCallsFilter(false);
  };
  ...
  { name: "People", value: 16, type: "loss" },
      ...
      <div className="p-4 pb-2 flex-grow">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center space-x-3">
            ...
            {title === 'Conversations' && <CheckCircle className="w-6 h-6" style={{ color: color }} />}
            {title === 'Calls Recorded' && <Mic className="w-6 h-6" style={{ color: color }} />}
        ...
        <div className="mb-3">
          ...
          {title === 'Conversations' && (
            <p className="text-sm text-gray-500">Total Conversations in last 4 weeks</p>
          )}
          {title === 'Calls Recorded' && (
            <p className="text-sm text-gray-500">Total calls recorded in last 4 weeks</p>
          )}
        </div>
        
        {/* 数字和占比 - 水平布局 */}
        <div className="flex items-end">
          <h4 className="text-3xl font-semibold text-gray-900 mr-3">{data.reduce((sum, item) => sum + item.value, 0)}</h4>
          <div>
            {title === 'Conversations' && (
              <p className="text-base text-gray-500">{percentage}% of Calls Completed</p>
            )}
            {title === 'Calls Recorded' && (
              <p className="text-base text-gray-500">{percentage}% of Conversations</p>
            )}
          </div>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="bg-white px-4 md:px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex flex-col lg:flex-row lg:items-center space-y-4 lg:space-y-0 lg:space-x-8 w-full">
            <div className="flex items-center space-x-4 lg:space-x-6">
              <h1 className="text-2xl font-bold text-gray-900">Calls Reports</h1>
      ...
      {/* Body */}
      <main className="px-6 py-6 overflow-auto space-y-2" style={{height: 'calc(100vh - 120px)'}}>
         ...
         {/* Appointments Card */}
          <div className="bg-white rounded-lg border border-gray-200 overflow-hidden flex flex-col h-full">
            <div className="p-4 pb-2 flex-grow">
              ...
              {/* 描述 */}
              <div className="mb-3">
                <p className="text-sm text-gray-500">Total appointments in last 4 weeks</p>
              </div>
              
              {/* 数字和说明 - 水平布局 */}
              <div className="flex items-end">
                <h4 className="text-3xl font-semibold text-gray-900 mr-3">{dealsWonData.reduce((sum, item) => sum + item.value, 0)}</h4>
                <p className="text-base text-gray-500">Appointments/Conversation</p>
              </div>
            </div>
            <div className="px-4 pt-2 pb-1">
              <ResponsiveContainer width="100%" height={90}>
          ...
          {/* Conversations Card */}
          <BarChartComponent data={appointmentsData} color="#60A5FA" title="Conversations" percentage={Math.round((appointmentsData.reduce((sum, item) => sum + item.value, 0) / 136) * 100)}/>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4 flex flex-col h-full">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-3">
                ...
                <p className="text-lg font-semibold text-gray-900">Calls Completed</p>
              </div>
              <div className="flex items-center space-x-1">
                <TrendingUp className="w-4 h-4 text-green-600" />
            ...
            <div className="mb-3 h-10 flex items-start">
              <p className="text-sm text-gray-500">Number of calls completed</p>
            </div>
          ...
          {/* Calls Recorded Card */}
          <BarChartComponent data={meetingsRecordedData} color="#FBBF24" title="Calls Recorded" percentage={Math.round((meetingsRecordedData.reduce((sum, item) => sum + item.value, 0) / appointmentsData.reduce((sum, item) => sum + item.value, 0)) * 100)} />
        </div>

        {/* Second Row - Win Rate, Win/Loss Insights, Calls Cancelled, Calls No-Show */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-2 mt-3">
          ...
          <div className="bg-white rounded-lg border border-gray-200 overflow-hidden flex flex-col h-full">
            ...
            <div className="flex-grow flex px-4 pb-4 relative">
              {/* 左侧容器 - 30%数字 - 绝对定位 */}
              <div className="absolute left-4 top-2 z-30 w-16">
                <h4 className="text-3xl font-semibold text-gray-900">30%</h4>
              </div>
              
              {/* 右侧容器 - 空白 - 绝对定位 */}
              <div className="absolute right-4 top-2 z-10 w-16">
                {/* 空白区域 */}
              </div>
              ...
              <PieChart>
                ...
                <div className="flex justify-center space-x-1 mt-0 flex-wrap">
          ...
          {/* Calls Cancelled Card */}
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            {/* 标题和趋势 */}
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-3">
                <CalendarX className="w-6 h-6 text-red-500" />
                <p className="text-lg font-semibold text-gray-900">Calls Wrong Number</p>
              </div>
              <div className="flex items-center space-x-1">
            ...
            {/* 描述 */}
            <div className="mb-3 h-10 flex items-start">
              <p className="text-sm text-gray-500">Contacts with invalid or outdated phone numbers</p>
            </div>
            ...
            {/* 占比 */}
            <div>
              <p className="text-base text-gray-500 pt-5">13% of Calls Completed</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-3">
                ...
                <p className="text-lg font-semibold text-gray-900">Calls No-Answer</p>
              </div>
              <div className="flex items-center space-x-1">
            ...
            {/* 描述 */}
            <div className="mb-3 h-10 flex items-start">
              <p className="text-sm text-gray-500">Calls where no one answered</p>
            </div>
            ...
            {/* 占比 */}
            <div>
              <p className="text-base text-gray-500 pt-5">6% of Calls Completed</p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
            <div className="flex items-center justify-between p-4 pb-2">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-lg font-semibold text-gray-900">High Performing Calls</p>
                </div>
              </div>
              <div className="relative">
                ...
                {showCallsFilter && (
                  <DashboardMeetingFilter
                    isOpen={showCallsFilter}
                    onClose={handleCloseCallsFilter}
                    onApply={(filters) => {
